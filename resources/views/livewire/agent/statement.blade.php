        ...
        @forelse($grouped as $dateLabel => $group)
            ...
            <div class="overflow-hidden rounded-xl border border-app-border bg-app-surface">
                @foreach ($group as $entry)
                    @php
                        $sp = \App\Services\Api\Support\StatementEntryPresenter::for($entry);

                        $isCredit = $entry['entryType'] === 'CREDIT';
                        $time = Carbon::parse($entry['postedAt'])->format('H:i');

                        $sign = $sp['sign'] ?? ($isCredit ? '+' : '−');
                        $signClass = $sp['text_class'] ?? ($isCredit ? 'text-app-green' : 'text-app-red');
                        $iconBgClass = $sp['bg_class'] ?? ($isCredit ? 'bg-app-green-bg' : 'bg-app-red-bg');
                        $label = $sp['label'] ?? ($entry['description'] ?? '—');
                    @endphp

                    <button wire:click="selectEntry('{{ $entry['id'] }}')" type="button" @class([
                        ...
                        {{-- Left icon circle --}}
                        <span
                            class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full {{ $iconBgClass }}">
                            @if ($isCredit)
