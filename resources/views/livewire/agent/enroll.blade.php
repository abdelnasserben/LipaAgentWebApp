<div>
    ...
    {{-- Progress stepper --}}
    <div class="border-b border-app-border bg-app-surface px-4 pt-5">
        <div class="mx-auto mb-4 flex max-w-xs items-center justify-center gap-0">
            @php
    ...
    <div class="px-4 py-5">
        ...
        @if($step === 1)
            <div>
                <div class="mb-5">
                    <div class="mb-1 text-[15px] font-bold text-app-text">Informations personnelles</div>
                    <div class="text-xs text-app-muted">Renseignez l'identité du client</div>
                </div>
        ...
        @elseif($step === 2)
            <div>
                <div class="mb-4">
                    <div class="mb-1 text-[15px] font-bold text-app-text">Adresse du client</div>
                ...
                <div class="mb-5 flex items-center gap-2 rounded-lg border border-app-blue bg-app-blue-bg px-3.5 py-2.5">
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" class="text-app-blue">
                ...
                @foreach([
                    ...
                    <div class="{{ $loop->last ? 'mb-6' : 'mb-3.5' }}">
                        <label class="mb-1.5 block text-[11px] font-bold uppercase tracking-[0.08em] text-app-muted">
                            {{ $field['label'] }}
                        </label>
                ...
                <div class="grid grid-cols-[1fr_2fr] gap-2.5">
                    <button wire:click="goToStep(1)" type="button"
                        class="flex cursor-pointer items-center justify-center gap-1.5 rounded-[10px] border-[1.5px] border-app-border bg-transparent p-3.5 text-sm font-semibold text-app-muted">
                        <x-agent-icon name="back" :size="14" />
                        Retour
                    </button>
                    <button wire:click="nextStep" type="button"
                        class="cursor-pointer rounded-[10px] border-0 bg-app-accent p-3.5 text-[15px] font-bold text-white">
                        Suivant →
                    </button>
                </div>
            </div>
        ...
        @elseif($step === 3)
            <div>
                <div class="mb-5">
                    <div class="mb-1 text-[15px] font-bold text-app-text">Documents KYC</div>
                ...
                <div class="mb-4 flex gap-2">
                    <select wire:model="kycDocType"
                        class="flex-1 cursor-pointer appearance-none rounded-lg border-[1.5px] border-app-border bg-app-surface px-3.5 py-3 text-[13px] text-app-text outline-none focus:border-app-accent">
                        <option value="NATIONAL_ID">Carte Nationale d'Identité</option>
                        <option value="PASSPORT">Passeport</option>
                    ...
                    <button wire:click="addKycDoc" type="button"
                        class="flex cursor-pointer items-center gap-1.5 whitespace-nowrap rounded-lg border-[1.5px] border-app-accent bg-app-accent-bg px-4 py-3 text-[13px] font-bold text-app-accent">
                ...
                @else
                    <div class="mb-4 rounded-[10px] border border-dashed border-app-border bg-app-surface p-6 text-center">
                ...
                <div class="grid grid-cols-[1fr_2fr] gap-2.5">
                    ...
                    <button wire:click="nextStep" wire:loading.attr="disabled" type="button"
                        class="flex cursor-pointer items-center justify-center gap-2 rounded-[10px] border-0 bg-app-accent p-3.5 text-[15px] font-bold text-white disabled:cursor-not-allowed disabled:opacity-70">
        ...
        @elseif($step === 4)
            ...
            @else
                <div>
                    ...
                    {{-- IDs --}}
                    <div class="mb-5 rounded-xl border border-app-border bg-app-surface px-4 py-1">
                    ...
                    @if($offerCardSale && count($cardStock) > 0)
                        <div class="mb-5 rounded-xl border-[1.5px] border-app-purple bg-app-surface p-4">
                            ...
                            <div class="mb-3.5 flex flex-col gap-2">
                                @foreach($cardStock as $card)
                                    ...
                                    <button wire:click="selectCard('{{ $card['nfcUid'] }}')" type="button"
                                        ...
                                        <div class="flex-1">
                                            <div class="font-mono text-xs font-bold text-app-text">
                                                {{ $card['internalCardNumber'] }}
                                            </div>
                                            <div class="mt-0.5 font-mono text-[11px] tracking-[0.05em] text-app-muted">
                                                NFC: {{ $card['nfcUid'] }}
                                            </div>
                                        </div>
                    ...
                    <button wire:click="finish" wire:navigate type="button"
                        class="flex w-full cursor-pointer items-center justify-center gap-2 rounded-[10px] border-0 bg-app-accent p-3.5 text-[15px] font-bold text-white">
