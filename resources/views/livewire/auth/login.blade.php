<div class="px-8 py-10">
    ...
    @if ($error)
        <x-alert variant="warning" :icon="false" class="mb-5" text-class="text-[13px] font-medium">
            {{ $error }}
        </x-alert>
    @endif
            ...
            <div class="mb-5">
                ...
                <div class="flex items-stretch gap-2">
                    ...
                    <input type="tel" wire:model="phoneNumber" placeholder="3XX XXXX" inputmode="numeric"
                        pattern="\d*" autocomplete="tel-national" autofocus
    ...
    @elseif($step === 'mfa')
        <div>
            ...
            <form wire:submit="verifyMfa">
                <div class="mb-6">
                    <label
                        class="mb-3 block text-center text-xs font-semibold uppercase tracking-[0.03em] text-app-muted">
                        Code TOTP a 6 chiffres
                    </label>

                    <input type="text" wire:model.live="totpCode" inputmode="numeric" pattern="\d*" maxlength="6"
                ...
                <button type="submit" wire:loading.attr="disabled" wire:target="verifyMfa"
                    class="flex w-full cursor-pointer items-center justify-center gap-2 rounded-[10px] border-0 bg-app-accent p-3.5 text-[15px] font-bold text-white disabled:cursor-not-allowed disabled:opacity-70">
    ...
    @else
        <div>
            ...
            <form wire:submit="setupPin">
                ...
                <button type="submit" wire:loading.attr="disabled" wire:target="setupPin"
                    class="flex w-full cursor-pointer items-center justify-center gap-2 rounded-[10px] border-0 bg-app-accent p-3.5 text-[15px] font-bold text-white disabled:cursor-not-allowed disabled:opacity-70">
