...
- **Forgotten-PIN self-service** is available **only** when the Agent has TOTP enrolled. Endpoint: `POST /api/v1/auth/agent/auth-pin/reset` (public; body `{ phoneCountryCode, phoneNumber, totpCode, newPin }`). The server verifies the TOTP code and, on success, atomically replaces `auth_pin_hash` with the BCrypt hash of `newPin` and resets the lock counters. The server never generates, returns or transmits a temporary PIN. Agents without TOTP get `422 AUTH_PIN_RESET_TOTP_REQUIRED` and must contact Backoffice for a forced reset (`POST /api/v1/backoffice/agents/{id}/auth-pin/reset`), which clears the PIN state and returns the Agent to the `pinSetupRequired` branch at next login. Unknown phone or wrong TOTP both return `401 AUTH_MFA_INVALID` (anti-enumeration). TOTP failures do not count toward the PIN lock policy.
...
| POST | `/api/v1/auth/agent/auth-pin/reset` | Public | none | none | `ResetAuthPinRequest` | `204 No Content` | `400 VALIDATION_FIELD_REQUIRED`, `401 AUTH_MFA_INVALID`, `422 AUTH_PIN_RESET_TOTP_REQUIRED`, `422 AUTH_PIN_FORMAT`, `422 ACTOR_SUSPENDED`, `422 ACTOR_CLOSED`, `422 ACTOR_PENDING_KYC` | Self-service forgotten-PIN reset gated by TOTP. New PIN supplied in the request; no temporary PIN is generated. |
